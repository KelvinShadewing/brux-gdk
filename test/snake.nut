...
::gmPlay <- function() {
	...
	if(getFrames() % max(10 - floor(snake.len() / 8), 1) == 0) {
		...
		if(snake[0][0] < 0) snake[0][0] = 39
		if(snake[0][0] > 39) snake[0][0] = 0
		if(snake[0][1] < 0) snake[0][1] = 29
		if(snake[0][1] > 29) snake[0][1] = 0
...
::gmOver <- function() {
	drawText(font2, 320 - (6 * 4.5), 234, "GAME OVER")
