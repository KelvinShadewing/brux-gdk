...
void sqDecodeJSONTable(HSQUIRRELVM v, cJSON *Item) {
	...
	while(Item) {
		if(Item->string)
			sq_pushstring(v, towc(Item->string), -1);
		switch(Item->type) {
			case cJSON_False:
			...
			case cJSON_Number:
				...
				break;
			case cJSON_String:
				sq_pushstring(v, towc(Item->valuestring), -1);
				break;
			case cJSON_Array:
				sq_newarray(v, 0);
...
SQInteger sqDecodeJSON(HSQUIRRELVM v) {
	...
	if(Str[0]!='{' && Str[0]!='[') {
		if(!strcmp(tostr(Str), "true"))
			sq_pushbool(v, SQTrue);
		else if(!strcmp(tostr(Str), "false"))
			sq_pushbool(v, SQFalse);
		else if(isdigit(Str[0]) || (Str[0]=='-' && isdigit(Str[1])))
			sq_pushinteger(v, strtol(tostr(Str), NULL, 0));
		else
			sq_pushstring(v, Str, -1);
		return 1;
	}

	cJSON *Root = cJSON_Parse(tostr(Str));
	if(!Root || !Root->child) {
		sq_pushnull(v);
		return 1;
	}
...
SQInteger sqLsDir(HSQUIRRELVM v) {
	...
	folder = opendir(tostr(dir));
	if(folder == NULL) {
		xyPrint(0, L"Failed to open directory: %s\n", dir);
		sq_pushstring(v, L"", 0);
		return 1;
	} else {
		sq_newarray(v, 0);
		while(entry = readdir(folder)) {
			...
			sq_pushstring(v, towc(s_entry.c_str()), s_entry.length());
			sq_arrayappend(v, -2);
		}
	}
