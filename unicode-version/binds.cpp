...
SQInteger sqGetOS(HSQUIRRELVM v) {
	switch(xyGetOS()) {
		case 0:
			sq_pushstring(v, L"windows", 7);
			break;
		case 1:
			sq_pushstring(v, L"mac", 3);
			break;
		case 2:
			sq_pushstring(v, L"linux", 5);
			break;
		case 3:
			sq_pushstring(v, L"gcwz", 4);
			break;
		case 4:
			sq_pushstring(v, L"pandora", 7);
			break;
		case 5:
			sq_pushstring(v, L"android", 7);
			break;
		case 6:
			sq_pushstring(v, L"ios", 7);
			break;
	}
...
SQInteger sqSetFPS(HSQUIRRELVM v) {
	...
	if(iMax < 0) {
		xyPrint(0, L"Maximum FPS cannot be negative.");
		return 0;
	} else gvMaxFPS = iMax;
...
SQInteger sqSetWindowTitle(HSQUIRRELVM v) {
	...
	sq_getstring(v, 2, &t);
	SDL_SetWindowTitle(gvWindow, tostr(t));
...
SQInteger sqLoadWindowIcon(HSQUIRRELVM v) {
	...
	sq_getstring(v, 2, &t);
	if(xyFileExists(tostr(t))) {
		SDL_Surface* icon = IMG_Load(tostr(t));
...
SQInteger sqImport(HSQUIRRELVM v) {
	...
	sqstd_dofile(gvSquirrel, towc(b.c_str()), 0, 1);
...
SQInteger sqDoString(HSQUIRRELVM v) {
	...
	sq_compilebuffer(gvSquirrel, a, (int)strlen(tostr(a)) * sizeof(SQChar), L"string", 1);
...
SQInteger sqFileExists(HSQUIRRELVM v) {
	...
	sq_pushbool(v, xyFileExists(tostr(file)));
...
SQInteger sqGetDir(HSQUIRRELVM v) {
	...
	sq_pushstring(v, towc(buff), strlen(buff));
	return 1;
};
...
SQInteger sqFileAppend(HSQUIRRELVM v) {
	...
	ofstream fi;
	fi.open(tostr(f), ios::out | ios::app);
	fi << tostr(s);
	fi.close();
...
SQInteger sqFileRead(HSQUIRRELVM v) {
	...
	if(!xyFileExists(tostr(f))) {
		xyPrint(0, L"WARNING: %s does not exist!", f);
		sq_pushstring(v, L"-1", 2);
		return 1;
	} else {
		t.open(tostr(f));
		t.seekg(0, ios::end);
...
SQInteger sqLoadImage(HSQUIRRELVM v) {
	...
	sq_pushinteger(v, xyLoadImage(tostr(file)));
...
SQInteger sqLoadImageKeyed(HSQUIRRELVM v) {
	...
	sq_pushinteger(v, xyLoadImageKeyed(tostr(file), key));
...
SQInteger sqSetResolution(HSQUIRRELVM v) {
	...
	if(w < 1 || h < 1) {
		xyPrint(0, L"Window dimensions cannot be 0.");
		return 0;
	}
...
SQInteger sqSpriteName(HSQUIRRELVM v) {
	...
	if(vcSprites.size() <= s || s < 0) sq_pushstring(v, L"N/A", 3);
	else if(vcSprites[s] == 0) sq_pushstring(v, L"N/A", 3);
	else sq_pushstring(v, towc(vcSprites[s]->name.c_str()), vcSprites[s]->name.length());
...
SQInteger sqNewSprite(HSQUIRRELVM v) {
	...
	xySprite* newSprite = new xySprite(tostr(i), w, h, m, p, px, py);
...
SQInteger sqPadName(HSQUIRRELVM v) {
	...
	sq_pushstring(v, towc(gvPadName[i].c_str()), gvPadName[i].length());
...
SQInteger sqLoadSound(HSQUIRRELVM v) {
	...
	sq_pushinteger(v, xyLoadSound(tostr(s)));
...
SQInteger sqLoadMusic(HSQUIRRELVM v) {
	...
	sq_pushinteger(v, xyLoadMusic(tostr(m)));
